...
<?php
  if (@$approved = $_REQUEST['rejected_details']) {

$sql = "SELECT * FROM employees JOIN resignation WHERE employees.id = resignation.emp_id AND resignation.r_id = '$approved' ";
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_assoc($result);
if ($row['team_m_approval']=='' || $row['team_m_approval']=='approved' || $row['hr_approval']=='approved' || ($row['team_m_approval']=='approved' AND $row['hr_approval']=='reject') ) {
 ?> 
