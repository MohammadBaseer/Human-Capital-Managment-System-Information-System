...
<h5>Department Employee's Leaves for Approval</h5>
...
<thead>
                    ...
                    $sql = "SELECT * FROM leave_forms as l INNER JOIN employees as e on l.employee_id = e.id  INNER JOIN department AS d ON e.department_id = d.departmentid WHERE  l.manager = '$owner' AND manager_sign = '' ";
     ...
     <tr>
                            ...
                            }elseif($row['manager_sign'] == "approved" && $row['hr_sign'] == "approved"){
                              echo "<span class='text-white label theme-bg f-12'> HR & Manager </span>";
                            }else{
                ...
                <td><!-- &nbsp; &nbsp;
                <a href="em-leave-form-sign.php?formedit=<?php //echo $row['id'];?>" data-toggle="tooltip" title="View"> 
                        <i class="feather icon-file" aria-hidden="true"></i>
                </a> -->&nbsp;&nbsp;
                <a href="controller/approval.php?manager_approved=<?php echo $row['leave_id'];?>" data-toggle="tooltip" title="Approve"> 
                        <i class="feather icon-check" aria-hidden="true"></i>
                </a>&nbsp;&nbsp;
                <a id="reject" href="controller/approval.php?manager_rejected=<?php echo $row['leave_id'];?>" onclick="return confirm('Are you sure..?');" data-toggle="tooltip" title="Reject">
                        <i class="feather icon-x" aria-hidden="true"></i>
                </a>&nbsp;&nbsp;
<a id="reject" href="leave_veiw.php?manager_approved=<?php echo $row['leave_id'];?>" data-toggle="tooltip" title="View">
